...
public class UserAdapterHelperFirebaseTest {
    ...
    public void loadUsers_waitlist_populatesUserList_andCleansUp() throws Exception {
        ...
        try {
            ...
            base.put("createdAt", Timestamp.now().toDate().getTime());

            Map<String, Object> u1data = new HashMap<>(base);
            u1data.put("username", u1);
            u1data.put("name", "User One");
            u1data.put("email", u1 + "@example.com");

            Map<String, Object> u2data = new HashMap<>(base);
